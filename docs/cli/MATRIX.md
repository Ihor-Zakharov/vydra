...
сайта), 2 KNOWN (ограничение, которое выдра объясняет).** Непройденных по вине выдры нет. Ещё 16 сценариев проверены вручную
...
| M13 | Tab-подсказки (bash, zsh) | `COMP_WORDS="vydra st" _VYDRA_COMPLETE=complete_bash vydra`, то же для zsh | **не работали и в выпущенной версии: «Shell bash not supported»** (классы оболочек Typer не регистрировались при своей команде `completion`); теперь `stop`, `показать`, `--json`, варианты `-f` | FIX 13d3d96 |
| M14 | `vydra update` на временном HOME | `.sprint/update_test.sh`: установка из клона → `vydra ui` → коммит «1.0.1» в клоне → `vydra update` без ключей; затем `--repo` на несуществующий адрес и на сломанную копию | «1.0.0 · eff6ad9 → 1.0.1 · b8960ff», ui перезапущен в том же окне и отвечает 1.0.1, Tab работает; сбои — понятная ошибка, установленная 1.0.1 цела. **Перезапуск из папки клона поднимал старую версию** (`python -m vydra` брал пакет из текущей папки) — теперь `python -I` | FIX eff6ad9 |
| M15 | Хранилище на 50 000 файлов (ext4) и 10 000 (/mnt/c) | `.sprint/biglib.py`, `.sprint/drvfs_scan.py` | весь список 12–27 с → 0,02–0,35 с, папка до 67 с → 0,13 с, индекс 15 → 2,4 с, очередь фона 1,35 млн → 50 тыс., изменение на /mnt/c 37 → 0,22 с | FIX |
| M16 | Перенос хранилища ext4 → /mnt/c с обрывом | `.sprint/move_test.py`: 200 файлов по 1 МБ, kill -9 через 4 с, повтор, обратно | всё на месте (sha1), id те же, без дублей и .part, чужое не тронуто. **Из интерфейса перенос не работал** (`move` игнорировался) — теперь работает | FIX |
