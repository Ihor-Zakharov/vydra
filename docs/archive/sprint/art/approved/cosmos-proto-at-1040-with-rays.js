...
//
// S1c: токены --bh-core, --bh-sea, --bh-sea-lens, --bh-fog-rot; туман в долях R (одинаков во всех окнах); оттенок источника ≤ 33 %;
// энергия загрузки мягче (время ×1,3); лучи без «отрицательного света». Параметры и правила — docs/design/DIRECTION.md §5.
//
...
uniform float uFib;
uniform float uCore;     // яркость плато раскалённого пояса (ниже — видны нити)
uniform float uSea;      // ширина светлой плоскости по x, доли R (0 — во всю ширину)
uniform float uSeaLens;  // линза для плоскости ниже линии горизонта 0..1 (1 — как у тумана)
uniform float uFogRot;   // поворот рисунка тумана, рад (подбор без симметричных «ушей»)
...
uniform float uHasFar;     // 0..1 — план проявляется за 0,8 с после загрузки (без скачка)
uniform float uExpo;       // экспозиция открытия 0..1 (1,4 с): сначала кольцо, потом туман
...
float haloCore(float e, float band){
  float plateau = smoothstep(0.012, 0.07, e) * (1.0 - smoothstep(band * 0.45, band * 1.25, e)) * uCore;
...
void main(){
  ...
  float breath = 1.0 + 0.03 * sin(t * TAU / 7.0) + 0.05 * uEnergy + 0.22 * uSwell;
  ...
  if (uHasFar > 0.001) {
    float belowF = smoothstep(0.05, -0.35, ylF);                    // плоскость: линза слабее, свет — пятном под дырой
    float lensP = mix(lensF, mix(1.0, lensF, uSeaLens), belowF);
    vec2 Pl = c + p * max(lensP, -0.5) * uR + mdisp * H + uPar * H * 0.006;
    far = plate(Pl);
    if (uSea > 0.0) far *= mix(1.0, mix(0.28, 1.0, exp(-p.x * p.x / (2.0 * uSea * uSea))), belowF);
  }
  ...
  // туман считается в долях R (масштаб окна l, R/H = 0,14): рисунок одинаков относительно кольца во всех окнах
  const float kF = 0.14;
  vec2 qf = rot(uFogRot) * (p * lensF) * kF + mdisp;
  float rq = length(qf) / kF;                                      // линзированный радиус, доли R
  float twist = 0.85 * log(max(rq, 0.6)) - t * 0.02 * (1.0 + uEnergy);
  vec2 fq = rot(twist) * qf;
  ...
  vec2 f2 = rot(-0.35) * (qf * 5.2 + par * 1.3) - vec2(0.010, 0.004) * t * (1.0 + 1.5 * uEnergy) + 11.0;
  ...
  float sh1 = fbm(rot(uFogRot) * dir * 3.1 + vec2(uSeed * 5.0, t * 0.008), 3);
  float shafts = max(mix(1.0, 0.18 + 1.7 * smoothstep(0.30, 0.78, sh1), uRays), 0.0);   // при лучах > 1 — без «отрицательного света»
  float T = 1.0;
  if (e > 0.05 && e < 4.0) {
    float acc = 0.0;
    for (int k = 0; k < 6; k++) {
  ...
  float tintK = clamp(uTintAmt * (0.20 + 0.40 * min(lit * 1.5, 1.0)), 0.0, 0.40);   // оттенок источника — едва заметный
  ...
  col *= uExpo * uExpo;                                            // открытие: яркое (кольцо) проявляется первым
  ...
  lum += part * 0.35;
  lum *= uInner * (1.0 + 0.10 * uSwell) * uExpo;
  ...
  vec3 ringC = vec3(ringR, ringG, ringB) * (1.05 + 0.20 * be) * (1.0 + 0.4 * uSwell) * min(1.0, uExpo * 2.5);
...
const DEF = {
  ...
  core: 1.55, sea: 0, seaLens: 1, fogRot: 0,
};
...
export function createCosmos(canvas, opts = {}) {
  ...
  const api = {
    ...
    _far: 0, _expo: reduced ? 1 : 0, _age: 0,
  };
  if (!gl) return api;
  ...
  for (const n of ['uRes', 'uH', 'uTime', 'uPar', 'uMouse', 'uScroll', 'uC', 'uR', 'uHalo', 'uBand', 'uCA', 'uBeam', 'uBeamAmt', 'uLensE',
    ...
    'uInner', 'uFib', 'uCore', 'uSea', 'uSeaLens', 'uFogRot', 'uInvert', 'uSeed', 'uFarTex', 'uFarMap', 'uFarAnchor', 'uHasFar', 'uExpo']) U[n] = gl.getUniformLocation(prog, n);
  ...
  api.readTokens = () => {
    ...
    const n = {
      ...
      core: num(cs, '--bh-core', DEF.core), sea: num(cs, '--bh-sea', DEF.sea), seaLens: num(cs, '--bh-sea-lens', DEF.seaLens), fogRot: num(cs, '--bh-fog-rot', DEF.fogRot),
    };
  ...
  const draw = () => {
    ...
    gl.uniform1f(U.uCore, tok.core);
    gl.uniform1f(U.uSea, tok.sea);
    gl.uniform1f(U.uSeaLens, tok.seaLens);
    gl.uniform1f(U.uFogRot, tok.fogRot);
  ...
  api._frame = (now) => {
    ...
    api._age += dt;
    api._expo = reduced ? 1 : smooth(Math.min(1, api._age / 1.4));                       // открытие сцены
    api._far = reduced ? hasFar : Math.min(hasFar, api._far + dt / 0.8);                 // план проявляется, не выскакивает
