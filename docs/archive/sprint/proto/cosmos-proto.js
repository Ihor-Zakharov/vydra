// выдра — ЧЕРНОВИК сцены S1d (арт-директор). API как у vydra/static/cosmos.js, плюс pulse(kind) и setEnergy(v).
// Стенд подменяет им /static/cosmos.js (.sprint/proto/rig.py). Исполнитель фундамента (S2) переносит это в cosmos.js.
//
// ОСНОВА — одобренный пользователем гибрид 09:37 (.sprint/art/approved/cosmos-proto-approved-0937.js): вид сцены не менять.
// К нему добавлено только то, что просил пользователь (docs/design/DIRECTION.md §5, §4), всё «едва заметно»:
//  • глубина: явные планы — дальние звёзды почти стоят → средняя пыль и туман на горизонте (между дырой и «океаном») →
//    ближний план (крупные расфокусированные частицы, клочья тумана); воздушная перспектива — у горизонта мягче;
//    «океан» в перспективе — штрихи и волны мельче и плотнее к горизонту, крупнее у нижнего края;
//  • движение: линзы у курсора нет — вместо неё параллакс планов от мыши (дыра неподвижна); медленные волны по «океану»;
//    импульс «Скачать» — мягкий вдох кольца (без черты и вспышки); появление сцены 1,8 с; reduced — без параллакса и импульсов.
// Токены глубины (--bh-par, --bh-sea-persp, --bh-waves, --bh-mist, --bh-near, --bh-dust) = 0 возвращают эталон попиксельно.
//
// Слои (сзади вперёд): «чистый план» пользователя (звёзды, туманность, плоскость-«океан»; честно линзируется) → процедурные
// звёзды → средняя пыль → туман (спиральный вихрь, подсвечен кольцом, лучи и объёмные тени) → туман на горизонте → ореол
// (плато + многоступенчатый спад, нити, тонкие концентрические кольца, латеральная хроматическая аберрация) →
// плоскость-горизонт (пятно света, отражение кольца) → анаморфный блик → тон-маппинг → ядро-сфера (в экранных значениях) →
// фотонное кольцо → ближний план → дизеринг.
...
const OPEN = 1.8;          // появление сцены, с: кольцо — за первые 40 %, всё остальное — плавно к концу
...
export function createCosmos(canvas, opts = {}) {
  ...
  const api = {
    ...
    // указатель −1..1 → только параллакс планов (линзы у курсора больше нет); при reduced не двигает ничего
    ...
    // импульс — мягкий вдох кольца: 'go' (старт загрузки) и 'done' (готово, слабее). Не чаще 3 раз в секунду;
    // повтор во время вдоха продлевает его, а не начинает заново — скачков нет.
    pulse(kind = 'go') {
      if (reduced) return;
      const now = clock();
    ...
    _age: 0, _expo: reduced ? 1 : 0, _expoRing: reduced ? 1 : 0, _far: 0,
  };
  if (!gl) return api;
  ...
  // текстура дальнего плана: грузится лениво, до загрузки сцена рисуется без неё (S2: <link rel="preload" as="image">)
  let hasFar = 0;
  ...
  api.readTokens();
  // черновик: CSS варианта подмешивается после загрузки — следим за <style> в <head> (в cosmos.js не переносить, §0)
  ...
  // черновик: без правок app.js — вдох по #go.fire, энергия и «готово» по острову (в cosmos.js — прямые вызовы из app.js, §5.5)
  ...
  const draw = () => {
    ...
    gl.uniform1f(U.uNear, tok.near);
    gl.uniform1f(U.uDust, tok.dust);
  ...
  api._frame = (now) => {
    ...
    api._last = now;
    if (!reduced) api._t += dt * (1 + 0.3 * api.energy);                              // при загрузке время сцены ×1,3
    ...
    api._expo = reduced ? 1 : smooth(x);
    api._expoRing = reduced ? 1 : smooth(Math.min(1, x / 0.4));
    api._far = reduced ? hasFar : Math.min(hasFar, api._far + dt / 0.5);
    ...
    else { api._ptS[0] = ease(api._ptS[0], api._pt[0], dt, 0.6); api._ptS[1] = ease(api._ptS[1], api._pt[1], dt, 0.6); }
    ...
    draw();
    const settled = reduced && api._far >= hasFar;
