// выдра — «космос»: чёрная дыра в реальном времени (WebGL, один фрагментный шейдер).
// Оригинальная работа: гравитационное линзирование звёздного поля, тонкий аккреционный диск с
// доплеровским усилением, слабый релятивистский джет, крепускулярные лучи и объёмное свечение.
// Дисциплина: монохромная сцена; цвет — только в эмиссии диска/джета (uAccent). Рендер в ½
// разрешения, пауза, когда вкладка скрыта или герой ушёл с экрана; без WebGL — статичная подложка.
...
uniform float uEnergy;     // 0..1 — энергия диска (скорость загрузки)
uniform float uFlash;      // 0..1 — вспышка фотонного кольца + импульс джета
uniform float uAbsorb;     // 0..1 — «проглотили ссылку»
uniform vec3  uAccent;     // электрический цвет эмиссии
...
float fbm(vec2 p, int oct){ float a = 0.5, s = 0.0; for (int i = 0; i < 6; i++){ if (i >= oct) break; s += a * vnoise(p); p = p * 2.03 + 17.1; a *= 0.5; } return s; }
...
void main(){
  ...
  // p — координаты относительно тени, радиус тени = 1
  vec2 p = (gl_FragCoord.xy - ctr) / (m * uScale);
  float b = length(p);
  float ang = atan(p.y, p.x);
  float t = uTime;

  // ---------- линзирование фона (тонкая линза, эйнштейновский радиус = 1.0) ----------
  float k = 1.0;
  float lens = 1.0 - k / (b * b + 0.02);
  vec2 q = p * lens;                                   // положение источника
  vec2 par = uPointer * 0.06;                          // параллакс указателя (слои — на разной глубине)
  float upp = 1.0 / (m * uScale);                      // единиц на пиксель (до линзы)
  float mag = clamp(abs(lens) + 2.0 * k / (b * b + 0.02) * (b / (b + 1e-3)) * 0.0, 0.05, 40.0);
  float uppq = upp * clamp(abs(lens), 0.08, 4.0);     // размер звезды в q-пространстве

  float L = 0.0;                                       // яркость (линейная, монохром)
  vec3 col = vec3(0.0);

  // ---------- звёзды: три слоя, глубокий/средний/ближний ----------
  ...
  // усиление яркости при увеличении (поверхностная яркость сохраняется)
  L *= clamp(1.0 / max(abs(lens), 0.12), 0.6, 3.0);

  // ---------- туманность: холодный газ, ≤ 5 % яркости, пустоты у центра ----------
  {
    vec2 nq = q * 0.9 + par * 0.4 + vec2(0.015, 0.006) * t + uSeed * 3.0;
    nq = mat2(0.9, 0.44, -0.44, 0.9) * nq;
    float n = fbm(nq * 0.55, 5);
    float mk = fbm(nq * 0.16 + 4.2, 3);
    float neb = smoothstep(0.42, 0.95, n) * smoothstep(0.38, 0.8, mk);
    float pocket = smoothstep(0.6, 2.4, b);            // у дыры газа меньше — пустота
    L += neb * 0.05 * pocket;
  }

  // ---------- аккреционный диск (наклон ~80°) ----------
  float cosI = 0.26;
  vec2 pd = vec2(p.x, p.y / cosI);
  float rd = length(pd);
  float ad = atan(pd.y, pd.x);
  float energy = 0.55 + 0.75 * uEnergy + 0.9 * uFlash;
  // кеплеровское вращение: внутренние кольца быстрее
  float w = 0.55 * (0.8 + 0.6 * uEnergy) / max(pow(rd, 1.5), 0.2);
  ...
  // доплер: левая сторона летит к нам — ярче и горячее
  float vlos = -0.36 * cos(ad);
  float beam = pow(1.0 + vlos, 2.4);
  float near = smoothstep(-0.3, 0.3, p.y);            // ближняя (нижняя) половина
  float disk = prof * tex * beam * (0.55 + 0.45 * near);
  // за тенью диск не виден
  disk *= smoothstep(0.98, 1.02, b);

  // ---------- дальняя сторона диска, перекинутая над и под тенью (линзирование) ----------
  float arch;
  {
    float ra = 1.0 + 0.11 + 0.035 * sin(ang * 3.0 + t * 0.2);
    float da = (b - ra) / (0.05 + 0.045 * abs(sin(ang)));
    float archTex = 0.55 + 0.8 * fbm(vec2(ang * 3.0 - t * 0.35, b * 6.0), 3);
    float top = smoothstep(0.05, 0.75, -p.y / max(b, 0.01));        // верхняя дуга — яркая
    float bottom = smoothstep(0.1, 0.9, p.y / max(b, 0.01)) * 0.4;  // нижняя — тоньше и слабее
    arch = exp(-da * da) * archTex * (top + bottom) * pow(1.0 - 0.36 * cos(ang), 2.0) * 1.9;
    arch *= smoothstep(0.99, 1.03, b);
  }

  // ---------- фотонное кольцо ----------
  float ring = exp(-pow((b - 1.025) / 0.014, 2.0)) * (0.9 + 2.2 * uFlash + 0.8 * uAbsorb);
  float ringHalo = exp(-pow((b - 1.03) / 0.09, 2.0)) * (0.10 + 0.5 * uFlash);

  // ---------- джет: тонкие лучи вдоль оси вращения ----------
  float jetX = exp(-p.x * p.x / (0.010 + 0.06 * abs(p.y)));
  float jetY = smoothstep(1.0, 1.9, abs(p.y)) * exp(-abs(p.y) * 0.3) * (p.y > 0.0 ? 1.0 : 0.55);
  float jetTex = 0.75 + 0.35 * fbm(vec2(p.x * 6.0, abs(p.y) * 2.5 - t * (1.4 + 4.0 * uFlash)), 3);
  float jet = jetX * jetY * jetTex * (0.05 + 0.10 * uEnergy + 1.1 * uFlash);

  // ---------- лучи (крепускулярные) и объёмное свечение диска ----------
  ...
  // тень
  col *= smoothstep(0.985, 1.0, b);
  // виньетка — к углам темнее
  vec2 vc = (gl_FragCoord.xy / res - 0.5) * vec2(res.x / res.y, 1.0);
  col *= 1.0 - 0.35 * dot(vc, vc);
  // тон-маппинг и дизеринг
  col = 1.0 - exp(-col * 1.35);
  col = pow(col, vec3(0.92));
  col += (ign(gl_FragCoord.xy + fract(t) * 17.0) - 0.5) / 255.0 * 2.5;
  // светлая тема: негатив, как астрономический отпечаток
  vec3 print = 1.0 - col;
  print = mix(print, vec3(0.955, 0.955, 0.945), 0.06);
...
export function createCosmos(canvas, opts = {}) {
  ...
  const api = {
    ok: false, energy: 0, accent: hexToRgb(opts.accent || '#6d8bff'), accentAmt: 1, invert: 0,
    center: opts.center || [0.68, 0.55], scale: opts.scale || 0.11,
    setEnergy(v) { api._energyTarget = Math.max(0, Math.min(1, v)); },
    flash(strength = 1) { api._flash = Math.max(api._flash, strength); api.wake(); },
    absorb() { api._absorb = 1; api.wake(); },
    ...
    setPointer(x, y) { api._pt = [x, y]; },
    setScroll(v) { api._scroll = v; },
    pause() { api._paused = true; },
    resume() { api._paused = false; api.wake(); },
    wake() { if (!api._raf && api.ok) api._raf = requestAnimationFrame(api._frame); },
    destroy() { cancelAnimationFrame(api._raf); api._raf = 0; },
    _flash: 0, _absorb: 0, _energyTarget: 0, _pt: [0, 0], _ptS: [0, 0], _scroll: 0, _paused: false, _raf: 0, _last: 0, _t: 0,
  };
  if (!gl) return api;
  ...
  try {
    ...
    // шейдер не собрался — остаёмся на CSS-подложке, но без ошибок в консоли
    return api;
  }
  gl.useProgram(prog);
  ...
  const U = {};
  for (const n of ['uRes', 'uTime', 'uPointer', 'uScroll', 'uCenter', 'uScale', 'uEnergy', 'uFlash', 'uAbsorb', 'uAccent', 'uAccentAmt', 'uInvert', 'uSeed']) U[n] = gl.getUniformLocation(prog, n);
  ...
  function resize() {
    ...
    const nw = Math.max(2, Math.round(r.width * dpr * RES_SCALE)), nh = Math.max(2, Math.round(r.height * dpr * RES_SCALE));
  ...
  const draw = () => {
    ...
    gl.uniform1f(U.uFlash, api._flash);
    gl.uniform1f(U.uAbsorb, api._absorb);
  ...
  api._frame = (now) => {
    ...
    // сглаживание: энергия, указатель, затухание вспышек
    api.energy += (api._energyTarget - api.energy) * Math.min(1, dt * 2.2);
    api._ptS[0] += (api._pt[0] - api._ptS[0]) * Math.min(1, dt * 3.5);
    api._ptS[1] += (api._pt[1] - api._ptS[1]) * Math.min(1, dt * 3.5);
    api._flash = Math.max(0, api._flash - dt * 0.9);
    api._absorb = Math.max(0, api._absorb - dt * 1.4);
    draw();
    const settled = reduced && api._flash === 0 && api._absorb === 0 && Math.abs(api._ptS[0] - api._pt[0]) < 0.002 && Math.abs(api._ptS[1] - api._pt[1]) < 0.002 && Math.abs(api.energy - api._energyTarget) < 0.005;
